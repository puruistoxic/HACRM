...
class CustomerService extends TenantService
{
    ...
    public function storeContactPerson(): void
    {
        $contactpersons = $this->getAttr('contactperson_information_details');

        $contactpersonInfo = [];

        foreach ($contactpersons as $key => $contactperson) {

            $contactpersonInfo[] = $this->contactpersonInfo($contactperson, $this->store()->id);
        }

        $this->store()
            ->contactpersons()
            ->insert($contactpersonInfo);
    }
    ...
    private function contactPersonInfo($contactperson, $customer_id): array
    {
        return [
            'customer_id' => $customer_id,
            'name' => $contactperson['name'],
            'country_id' => $contactperson['country_id'] ?? null,
            'city' => $contactperson['city'] ?? null,
            'zip_code' => $contactperson['zip_code'] ?? null,
            'area' => $contactperson['area'] ?? null,
            'details' => $contactperson['details'],
            'created_at' => now(),
            'updated_at' => now()
        ];
    }
    ...
    public function updateContactPerson(): void
    {
        $this->getModel()->contactpersons()->delete();
        ...
        foreach ($contactpersons as $key => $contactperson) {
            ...
            $contactpersonInfo[] = $this->contactpersonInfo($contactperson, $this->getModel()->id);
        }
