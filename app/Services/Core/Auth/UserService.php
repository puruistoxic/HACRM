...
use App\Models\Core\Auth\LoginLog;
...
class UserService extends BaseService
{
    ...
    protected $role;
    protected $branchOrWarehouse;

    public function __construct(User $user, Role $role, BranchOrWarehouse $branchOrWarehouse)
    {
        $this->model = $user;
        $this->role = $role;
        $this->branchOrWarehouse = $branchOrWarehouse;
    }
    ...
    public function login(Request $request)
    {
        ...
        //1 is default admin
        if ($user->id !== 1 and $user->branch_or_warehouse_id === null)
            throw new AuthenticationException(trans('default.no_branch_or_warehouse_is_found'));

        $user->branch_or_warehouse_id ? $bwStatus = $this->branchOrWarehouse::checkBranchOrWarehouseIsActive($user->branch_or_warehouse_id) : '';

        if ($user->id !== 1 and $bwStatus === false)
            throw new AuthenticationException(trans('default.branch_or_warehouse_is_inactive'));
        ...
        if (Hash::check(request()->get('password'), optional($user)->password)) {
            ...
            $latitude = $longitude = $city = $regionName = $country = $zip_code = null;

            $my_ip = $request->ip();

            $geoData = json_decode(file_get_contents("http://ip-api.com/json/{$my_ip}"), true);
            ...
            // list($latitude, $longitude) = $this->getUserCoordinates($my_ip);
            $user->last_login_latitude = $latitude;
            $user->last_login_longitude = $longitude;
            $user->city = $city;
            ...
            LoginLog::create([
                'user_id' => $user->id,
                'login_ip' => $request->ip(),
                'last_login_latitude' => $latitude,
                'last_login_longitude' => $longitude,
                'city' => $city,
    ...
    // protected function getUserCoordinates($my_ip)
    // {
    //     // Attempt to get the user's coordinates using Geolocation API
    //     $latitude = $longitude = null;


    //     // if (isset($_SERVER['HTTP_CF_IPCOUNTRY']) && $_SERVER['HTTP_CF_IPCOUNTRY'] === 'XX') {
    //     //     // In case the user is using a VPN or proxy, return null
    //     //     return [null, null];
    //     // }
    ...
    //     // Attempt to get coordinates using free API (replace with your preferred provider)
