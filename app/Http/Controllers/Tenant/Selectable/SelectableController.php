...
class SelectableController extends Controller
{
    ...
    public function selectableCustomers(): LengthAwarePaginator
    {
        ...
        return Customer::query()
            ...
            ->select(['id', 'first_name', 'customer_group_id'])
            ->orderBy('first_name', 'asc')
            ->paginate(request()->per_page ?? 150);
    }
    ...
    public function selectableContactPersons()
    {
        $selected = request()->get('selected');
        $orderBy = request()->get('order_by');
        ...
        return ContactPerson::query()
            ...
            ->filters(new ContactPersonFilter())
            ->select('id', 'name')
            ->paginate(request()->per_page ?? 150);
    }
