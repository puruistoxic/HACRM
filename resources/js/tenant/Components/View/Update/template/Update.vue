<template>
    <div v-if="!loading">
        <div class="mb-primary">
            ...
            <app-note :class="'mb-primary clearfix'" :title="$fieldTitle('update')" content-type="html" :notes="[Object.keys(no_update_message).length ?
                `${no_update_message.message_1} ${no_update_message.message_2 ? `\n ${no_update_message.message_2}` : ''}`
                : $t('update_warning_details')]" />
        </div>

        <app-modal v-if="defaultUpdateSuccess" modal-id="default-update-success" @close-modal="redirectHomePage"
            ...
            <div class="p-primary d-flex justify-content-center align-items-center flex-column">
                ...
                <button class="btn btn-primary" type="button" data-dismiss="modal" @click.prevent="redirectHomePage">
                    {{ $t('ok') }}
                </button>
        ...
        <div v-if="defaultUpdate && updates.result.length">
            <div
                ...
                <a href="https://salex.gainhq.com/documentation/change-log.html" target="_blank"
                    class="text-muted hover-underline font-size-90">{{ $t('change_logs') }}</a>
                <button type="button" class="btn btn-primary mt-primary" @click="updateApp(nextUpdatableVersion)">
                    {{ $t('update') }}
        ...
        <app-manual-updater v-if="!defaultUpdate" :next-version="nextVersion" @after-manual-update="redirectHomePage" />

        <app-update-confirmation-modal v-if="confirmationModalActive" modal-id="app-confirmation-modal"
            @confirmed="confirmed" @cancelled="cancelled" :message="$t('this_will_update_entire_app')" />
