<template>
  <div class="content-wrapper">
    <app-breadcrumb
      :page-title="ucWords($t('product_details'))"
            ...
            <div class="col-lg-8 col-xl-9">
              ...
              <div class="row justify-content-lg-between">
                <div class="col-lg-5 mb-5 mb-lg-0">
                  ...
                  <div class="row">
                    <div class="col-5">
                      <p class="text-muted">{{ $t('sku') }}</p>
                    </div>
                    <div class="col-7">
                      <p>
                        {{ product.sku ? product.sku : $t('not_added_yet') }}
                      </p>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-5">
                      <p class="text-muted">{{ $t('promotrade') }}</p>
                    </div>
                    <div class="col-7">
                      <p>
                        {{ product.promotrade ? product.promotrade : $t('not_added_yet') }}
                      </p>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-5">
                      <p class="text-muted">{{ $t('promoretail') }}</p>
                    </div>
                    <div class="col-7">
                      <p>
                        {{ product.promoretail ? product.promoretail : $t('not_added_yet') }}
                      </p>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-5">
                      <p class="text-muted">{{ $t('itanal') }}</p>
                    </div>
                    <div class="col-7">
                      <p>
                        {{ product.itanal ? product.itanal : $t('not_added_yet') }}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div class="col-lg-6">
                    ...
                    <div class="col-8">
                      <template v-if="product?.variants[0]?.tax">
                        <p>
                          {{
                            product?.variants?.length
                              ? product.variants[0]?.tax?.percentage
                              : '-'
                          }}
                        </p>
                      </template>
                      ...
                      <p>
                        {{
                          product?.product_type === 'variant'
                            ? $t('mixed')
                            : product?.variants?.length
                            ? product.variants[0]?.stock_reminder_quantity
                            : '-'
                        }}
                      </p>
                    </div>
                  ...
                  <div class="row">
                    <div class="col-4">
                      <p class="text-muted">{{ $t('vegetarian') }}</p>
                    </div>
                    <div class="col-8">
                      <p style="text-transform: capitalize !important">
                        {{
                  ...
                  <div class="row">
                    <div class="col-4">
                      <p class="text-muted">{{ $t('toplines') }}</p>
                    </div>
                    <div class="col-8">
                      <p style="text-transform: capitalize !important">
                        {{
                   ...
                   <div class="row">
                    <div class="col-4">
                      <p class="text-muted">{{ $t('additionallines') }}</p>
                    </div>
                    <div class="col-8">
                      <p style="text-transform: capitalize !important">
                        {{
                ...
                <template v-if="product.product_type === 'single'">
                  ...
                  <div class="col-lg-6">
                    ...
                    <div class="row">
                      <div class="col-4">
                        <p class="text-muted">{{ $t('retail_price') }}</p>
                      </div>
                      <div class="col-8">
                        <p>
                          ${{
                    ...
                    <div class="row">
                      <div class="col-4">
                        <p class="text-muted">{{ $t('trade_price') }}</p>
                      </div>
                      <div class="col-8">
                        <p>
                          ${{ product?.trade?.length ? product?.trade : '-' }}
                        </p>
                      </div>
                    </div>
                  </div>
    ...
    <app-confirmation-modal
      ...
      :modal-class="
        this.product.status.name === 'status_inactive' ? 'primary' : 'warning'
      "
      :icon="
        this.product.status.name === 'status_inactive'
          ? 'check-square'
          : 'slash'
      "
      :message="
        this.product.status.name === 'status_inactive'
          ? $t('you_are_going_to_activate_a_product')
          : $t('you_are_going_to_deactivate_a_product')
      "
...
import { PRODUCTS, PRODUCT_LIST } from '../../../../Config/ApiUrl-CP'
...
export default {
  ...
  computed: {
    ...
    visibleTaxWarranty() {
      ...
      const warranty_duration = [
        ...new Set(_.map(this.product.variants, 'warranty_duration')),
      ]
      const low_stock_quantity = [
        ...new Set(_.map(this.product.variants, 'low_stock_quantity')),
      ]
      return (
        warranty_duration.length === 1 &&
        taxes.length === 1 &&
        low_stock_quantity.length === 1
      )
    },
    productPhotos() {
  ...
  methods: {
    deleteProduct() {
      axiosDelete(this.productDeleteUrl)
        ...
        .catch((error) =>
          error.response ? this.toastException(error.response.data) : null
        )
    },
    cancelProductDelete() {
    ...
    getProduct() {
      this.preloader = true
      axiosGet(`${PRODUCTS}/${this.productId}/details`)
        .then(({ data }) => {
          this.product = data
      ...
      switch (action.type) {
        case 'show_stock_overview':
          return window.location.replace(
            urlGenerator(
              `inventory/stock/overview/variant/${row.variants[0].id}`
            )
          )
          break
        case 'change-status':
