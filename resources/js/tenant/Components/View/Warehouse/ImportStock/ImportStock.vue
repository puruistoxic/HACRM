    ...
    <form class="card border-0 card-with-shadow" data-url="" ref="form">
      <div class="card-body row">
        ...
        <div class="form-element col-12 mb-4">
          ...
          />
          <p class="mt-1">
            <span class="text-muted pr-5">{{
              $t('allowed_file_types_csv')
            }}</span>
            <a href="#" class="text-underline" @click="sampleDownload">{{
        ...
        <div class="col-12 col-md-6 mt-5">
          ...
          <button class="btn btn-primary" @click.prevent="handleUpload">
            {{ $t('upload') }}
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    formatDate(dateObj) {
      ...
      return `${date.year}-${date.month <= 9 ? '0' : ''}${date.month}-${
        date.day <= 9 ? '0' : ''
      }${date.day}`
    },
    handleUpload() {
        ...
        for (const key in this.formData) {
          if (key !== 'file') {
            if (key === 'purchase_date') {
              formData.append(
                key,
                this.formatDate(new Date(this.formData.purchase_date))
              )
            } else {
