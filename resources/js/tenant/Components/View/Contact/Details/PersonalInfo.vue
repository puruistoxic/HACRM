    ...
    <div class="col-lg-9">
      ...
      >
        ...
        <div class="form-group row">
          ...
          <div
            class="customer-group-input mb-3 col-md-6"
            v-if="Boolean(formData.customer_group_id)"
          >
...
export default {
  ...
  created() {
    axiosGet(`${CUSTOMERS}/${this.props}`)
      .then((response) => {
        this.formData = response.data
        if (response.data.status_logs && response.data.status_logs.length > 0) {
          this.formData.status = response.data.status_logs[0].status
        }
