...
import {addChooseInSelectArray, addSelectInSelectArray} from "../../../common/Helper/Support/FormHelper";
    ...
    methods: {
        ...
        getData() {
            ...
            axiosGet(this.fetchUrl).then(response => {
                if (this.chooseLabel || this.selectLabel) {
                    const valueField = this.$attrs['list-value-field'] ? this.$attrs['list-value-field'] : 'name';
                    this.arrays = this.chooseLabel ? addChooseInSelectArray(response.data, valueField, this.chooseLabel) : addSelectInSelectArray(response.data, valueField, this.selectLabel)
                } else {
                    this.arrays = response.data
                }
                this.$emit('list', this.arrays)
            }).finally(() => {
                this.loading = false;
            })
        }
