...
import {
  addChooseInSelectArray,
  addSelectInSelectArray,
} from '../../../common/Helper/Support/FormHelper'
...
export default {
  ...
  methods: {
    ...
    getData() {
      this.loading = true
      axiosGet(this.fetchUrl)
        .then((response) => {
          if (this.chooseLabel || this.selectLabel) {
            const valueField = this.$attrs['list-value-field']
              ? this.$attrs['list-value-field']
              : 'name'
            this.arrays = this.chooseLabel
              ? addChooseInSelectArray(
                  response.data,
                  valueField,
                  this.chooseLabel
