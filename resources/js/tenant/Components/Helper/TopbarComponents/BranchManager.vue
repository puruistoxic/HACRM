<template>
  <div v-if="this.$can('view_branch_or_warehouses')">
    ...
    <div
      v-else
      class="dropdown dropdown-with-animation d-inline-block btn-dropdown btn-tenant-dropdown"
    >
      <template>
        <!-- <button
          v-if="branch_or_warehouses.length"
          type="button"
          class="btn"
        ...
        >
          <span class="d-none d-md-inline-block mr-2">
            {{
              currentBranchOrWarehouse.id
                ? currentBranchOrWarehouse.name
                : $t('all')
            }}
          </span>
      ...
      <div class="dropdown-menu p-0">
        ...
        >
          <a
            v-if="role_ids.includes(roles[0].id)"
            href="#"
          ...
          <p
            ...
            :class="{
              'primary-text-color':
                branch_or_warehouse.id === currentBranchOrWarehouse
                  ? currentBranchOrWarehouse.id
                  : '',
              separator: branch_or_warehouse.last_branch,
            }"
...
export default {
  ...
  methods: {
    getBranchAndWarehouses() {
      axiosGet(`${BRANCHES_OR_WAREHOUSES}?active=true`)
        .then((response) => {
          this.branch_or_warehouses = response.data.data
          let currentBranchWahrehouseId = localStorage.getItem(
            'currentBranchWahrehouseId'
          )
          let currentBranchWahrehouseName = localStorage.getItem(
            'currentBranchWahrehouseName'
          )
          if (currentBranchWahrehouseId) {
            this.setBranchOrWarehouseId(eval(currentBranchWahrehouseId))
            this.setBranchOrWarehouseName(
              currentBranchWahrehouseName === 'null'
                ? null
                : currentBranchWahrehouseName
            )
            return
          }
          const userBranch = this.branch_or_warehouses.find(
            (bw) => bw.id === window.user.branch_or_warehouse_id
          )
    ...
    changeBranchOrWarehouse(branch_or_warehouse) {
      ...
      localStorage.setItem(
        'currentBranchWahrehouseId',
        eval(branch_or_warehouse.id)
      )
      localStorage.setItem(
        'currentBranchWahrehouseName',
        branch_or_warehouse.name
      )
      ...
      if (window.location.pathname.includes(salesViewPathName))
        return window.location.replace(urlGenerator('/dashboard'))
    },
  ...
  computed: {
    ...mapGetters([
      ...
      'getBranchOrWarehouseName',
      'getStocksByBranchOrWarehouse',
    ]),
    branchAndWarehouseIdsSorted() {
      if (!this.branch_or_warehouses) return []
      const branches = this.branch_or_warehouses.filter(
        (branchOrWarehouse) => branchOrWarehouse.type === 'branch'
      )
      const warehouses = this.branch_or_warehouses.filter(
        (branchOrWarehouse) => branchOrWarehouse.type === 'warehouse'
      )
